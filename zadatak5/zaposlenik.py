def require_num(metoda):
    def brojka(*args, **kwargs):
        if isinstance(args[1], int) or isinstance(args[1], float):
            rezultat = metoda(*args, **kwargs)
            return rezultat
        else:
            raise TypeError('krivi tip') 
    return brojka

def kapitaliziraj(metoda):
    def velika(*args, **kwargs):
        maliString = metoda(*args, **kwargs)
        return maliString.upper()
    return velika
